...
def _build_prophet(
    ...
    cp_scale = scale_override if scale_override is not None else config.get("changepoint_prior_scale", 0.05)
...
def _fit_predict(m: Prophet, df: pd.DataFrame, config: dict, periods: int) -> pd.DataFrame:
    growth = config.get("growth", "linear")
    fit_df = df.copy()
    if growth == "logistic":
    ...
    if len(val_df) >= 5:
        ...
        if mape > _MAPE_THRESHOLD:
            ...
            logger.warning(
                ...
                metric_type, mape, _MAPE_THRESHOLD, reduced_scale,
            )
            m_final = _build_prophet(config, scale_override=reduced_scale, extra_changepoints=extra_changepoints)
        else:
...
def generate_llm_report(metric_type, peak_value, expected_time, confidence, threshold, is_recovered=False, impact=None):
    ...
    if "cpu" in metric_type:
        return _get_cpu_report(peak_value, time_msg, conf_msg, is_risky, threshold, is_recovered)
...
def _get_cpu_report(peak, time_msg, conf_msg, is_risky, threshold, is_recovered=False):
    ...
    if not is_risky:
        return f"✅ [CPU 안정] 현재 사용량 {peak:.1f}%로 임계치({threshold}%) 미만입니다."
    return (
...
def _get_stabilization_report(metric, impact, is_risky, threshold):
    ...
    if not is_risky:
        opinion_msg = f"💡 [Opinion] 임계치({threshold}) 이하로 안정적으로 유지되고 있습니다."
    else:
