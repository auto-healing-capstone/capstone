...
def _do_calibration(metric_type: str, metric_name: str, threshold: float, recovered_at: datetime) -> None:
    """
    복구 후 STABILIZATION_WINDOW_MIN 대기 → 메트릭 수집 → 안정화 판단 → 결과 저장.
    별도 스레드에서 실행됨.
    """
    logger.info("[Calibrator] %s 보정 시작 (복구시각=%s, %d분 대기)",
                metric_type, recovered_at.isoformat(), STABILIZATION_WINDOW_MIN)
    ...
    if not raw:
        logger.warning("[Calibrator] %s post-recovery 데이터 없음, 보정 건너뜀", metric_type)
    ...
    if stabilized:
        ...
        logger.warning("[Calibrator] %s 아직 불안정 (mean=%.3f, threshold=%.1f)",
                       metric_type, mean_val or 0, threshold)
        _store_result(
            metric_type,
            stabilized=False,
...
def _retrain_with_changepoint(metric_type: str, metric_name: str, recovered_at: datetime) -> None:
    ...
    logger.info("[Calibrator] %s 재학습 완료 (changepoint=%s)", metric_type, recovered_at.isoformat())
